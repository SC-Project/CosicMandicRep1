package equinoobstim1.pluginDialog;
...
import equinoobstim1.pluginMenager.Plugin;
import equinoobstim1.pluginMenager.PluginMenager;
import equinoobstim1.pluginMenager.SelectionListener;

public class PluginMenagerDialog extends Dialog {
	...
	public PluginMenagerDialog(Shell parentShell) {
		super(parentShell);
	...
	protected Control createDialogArea(Composite parent) {
		...
		composite.getShell().setText("Plugin menager");
	...
	protected void initButtons(Composite composite){
		...
		addButton.setText(PluginMenager.start);
		addButton.addSelectionListener(new SelectionListener(tableViewer, m_bindingContext));
		
		Button removeButton = new Button(buttonComposite, SWT.PUSH); 
		removeButton.setText(PluginMenager.stop);
		removeButton.addSelectionListener(new SelectionListener(tableViewer, m_bindingContext));
		
		Button uninstallButton = new Button(buttonComposite, SWT.PUSH);
		uninstallButton.setText(PluginMenager.uninstall);*/
	}
	...
	protected DataBindingContext initDataBindings() {
		...
		IObservableMap[] observeMaps = PojoObservables.observeMaps(
				setContentProvider.getKnownElements(), Plugin.class,
				new String[] { "name", "status"});
		tableViewer.setLabelProvider(new ObservableMapLabelProvider(observeMaps));
		
			
		ArrayList<Plugin> plugins = PluginMenager.getPluginMenger().getPlugins().getPlugins();
		ArrayList<Plugin> equiNOOBSplugins = new ArrayList<Plugin>();
		...
		for (Plugin plugin : plugins) {
			...
			if(p.getName().contains("EquiNOOBSTim1"))
				equiNOOBSplugins.add(p);
			
			
		}
		
		//tableViewer.setLabelProvider(AddressBookLabelProvider);
			
		WritableList writableSet = new WritableList(equiNOOBSplugins, Plugin.class);
