...
public class FindDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		composite.getShell().setText("Pretraga");
		...
		utakmicaButton.setText("Utakmice");
		...
		utakmica = new JRadioButton("Utakmice", true);
	...
	public void find() {
		...
		if (timButton.getSelection() == true) {
			...
			if (t != null)
				PrvenstvoContainer.getInstance().pretragaSelekcijaTim(t);
			else
		...
		if (utakmicaButton.getSelection() == true) {
			...
			if (!utakmice.isEmpty() && utakmice != null)
				PrvenstvoContainer.getInstance().pretragaSelekcijaUtakmice(
						utakmice);
			else
				MessageDialog.openWarning(
						Display.getCurrent().getActiveShell(), "!",
