...
public class EquiZoomManager{
	 ...
	 */
	public EquiZoomManager(ScalableFigure pane, Viewport viewport) {
		this.pane = pane;
	 ...
	 */
	public EquiZoomManager(ScalableFreeformLayeredPane pane, Viewport viewport) {
		this.pane = pane;
		this.viewport = viewport;
	}
	...
	protected void primSetZoom(double zoom, Point mouse) {
		Point topleft = new Point();
		topleft.setX((int) (mouse.x*zoom + getViewport().getClientArea().width/2));
		topleft.setY((int) (mouse.y*zoom + getViewport().getClientArea().height/2));
		Point p1 = topleft;
		Point p2 = p1.getCopy();
		Point p = getViewport().getViewLocation();
		double prevZoom = this.zoom;
	...
	public void setZoom(double zoom, Point mouse) {
		currentZoomContant = null;
		zoom = Math.min(getMaxZoom(), zoom);
		zoom = Math.max(getMinZoom(), zoom);
		if (this.zoom != zoom) {
			primSetZoom(zoom, mouse);
		}
	}
	...
	public void zoomIn(Point mouse) {
		setZoom(getNextZoomLevel(), mouse);
	}
	...
	public void performAnimatedZoom(Rectangle rect, boolean zoomIn, int iterationCount) {
		double finalRatio;
		...
		System.out.println("Tacka koju prosledjujem: " + rect.x + ", " + rect.y);
		if(rect.x()<0)
			rect.setX(0);
		else if(rect.y()<0)
			rect.setY(0);
		getScalableFigure().translateToRelative(rect);
		System.out.println("Rect posle translacije: " + rect.x + ", " + rect.y);
		Point originalViewLocation = getViewport().getViewLocation();
		System.out.println("Original: " + originalViewLocation);
		Point finalViewLocation = calculateViewLocation(rect, finalRatio);
		System.out.println("Final: " + finalViewLocation + "\n\n");
